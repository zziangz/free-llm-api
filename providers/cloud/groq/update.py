#!/usr/bin/env python3
"""
Groq Model Registry Updater

Generates config.yaml by combining:
1. Model data from Groq /v1/models API
...
def fetch_models() -> List[Dict]:
    """Fetch latest models from Groq API"""
...
def generate_config(models: List[Dict], metadata: Dict) -> Dict:
    ...
    audio_models = set(metadata.get('audio_models', []))
    tts_models = set(metadata.get('tts_models', []))
    guard_models = set(metadata.get('guard_models', []))
    compound_models = set(metadata.get('compound_models', []))
    ...
    config = {
        ...
        'limits': {
            ...
            'tokens': limits.get('tokens', {'perMinute': 18000})
        },
        'models': []
    }
    ...
    for model in models:
        ...
        if model_id in audio_models:
            continue  # Whisper - audio transcription
        if model_id in tts_models:
            continue  # Orpheus - text-to-speech
        if model_id in guard_models:
            continue  # Safety classifiers
        
        context_window = model.get('context_window', defaults.get('contextWindow', 131072))
        max_output = model.get('max_completion_tokens', defaults.get('maxOutputTokens', 8192))
        ...
        has_tools = model_id in function_calling
        has_json = model_id in json_mode
        has_vision = model_id in vision_models
        is_free = model_id in free_models or len(free_models) == 0  # All free if not specified
...
def write_config(config: Dict):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write('# Groq uses OpenAI-compatible API\n')
        f.write('endpoint:\n')
    ...
    compound_count = sum(1 for m in config['models'] if m.get('type') == 'compound')
    
    print(f"  - Models with tools: {tools_count}")
    print(f"  - Compound models: {compound_count}")
